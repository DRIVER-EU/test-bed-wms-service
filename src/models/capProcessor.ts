...
import {Feature, GeoJsonProperties, GeoJsonGeometryTypes, GeometryObject} from 'geojson';
...
export class CapProcessor {
    ...
    public handleIncomingCAP(capAlert: Alert) {
        ...
        var fts: Feature<GeometryObject, GeoJsonProperties>[] = [];
        if (!capAlert.info) {
    ...
    private parseInfo(capInfo: Info, capAlert: Alert): Feature<GeometryObject, GeoJsonProperties>[] {
        var fts: Feature<GeometryObject, GeoJsonProperties>[] = [];
        var f = CapProcessor.createFeature();
        ...
        capAreas.forEach((area: Area) => {
            ...
            f.properties = CapProcessor.flattenCapProperties(f.properties, area);
            f.properties = CapProcessor.flattenCapProperties(f.properties, capInfo);
            f.properties = CapProcessor.flattenCapProperties(f.properties, capAlert);
            fts.push(f);
        });
        return fts;
    }
    ...
    private static flattenCapProperties(props: _.Dictionary<any>, capThing: Area | Info | Alert) {
        let capThingStringOnly = _.mapObject(capThing, (val, key) => {
            if (CapProcessor.blacklistProps.indexOf(key) >= 0) return null;
    ...
    private static createFeature(): Feature<GeometryObject, GeoJsonProperties> {
        return <Feature<GeometryObject, GeoJsonProperties>>{
            type: 'Feature',
            id: CapProcessor.newGuid(),
            properties: {},
            geometry: null
        };
    }
    ...
    public static createDefaultAlertMessage(): Alert {
        var alertMsg: any = {
            identifier: 'CSWEB',
            sender: 'csCOP',
            sent: CapProcessor.convertDateToCAPDate(new Date()), //'2016-03-31T11:33:00+02:00',//(new Date().toISOString()).replace('Z','+02:00'),
            status: Status.Test,
    ...
    public convertGeoJSONToCAPGeometry(
        geo: {type: string; coordinates: any},
        radiusKM: number = 10
    ): {
        key: string;
        val: string;
